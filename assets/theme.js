...
updateFacets_fn = function () {
  this.querySelector('[is="facets-form"]')?.dispatchEvent(
...
var Header = class extends HTMLElement {
  constructor() {
    ...
    __privateAdd(
      this,
      _onScrollListener,
      throttle(__privateMethod(this, _onScroll2, onScroll_fn2).bind(this))
    );
    __privateAdd(this, _offset, 10);
  }
  connectedCallback() {
    if (
      window.CSS.supports("selector(:has(.foo))") &&
      document.querySelector(
        ":has(.shopify-section:first-child [allow-transparent-header]):not(:has(.shopify-section--header + .shopify-section-group-header-group))"
      )
    ) {
      window.addEventListener("scroll", __privateGet(this, _onScrollListener));
      new ResizeObserver(
        __privateMethod(this, _recalculateOffset, recalculateOffset_fn).bind(
          this
        )
      ).observe(this);
    }
...
onScroll_fn2 = function () {
  this.classList.toggle(
    "is-solid",
    window.scrollY > __privateGet(this, _offset)
  );
};
...
recalculateOffset_fn = function () {
  ...
  if (previousSibling.classList.contains("shopify-section--announcement-bar")) {
    const computedStyles = getComputedStyle(document.documentElement),
      isAnnouncementBarSticky = parseInt(
        computedStyles.getPropertyValue("--announcement-bar-is-sticky") || 0
      );
...
document.querySelector(".see-more").addEventListener("click", function () {
  const extraContent = document.querySelector(".additional");
  const arrow = document.querySelector(".see-more .arrow");
  const seeMoreText = document.querySelector(".see-more .text");

  if (extraContent.style.display === "block") {
    extraContent.style.display = "none";
    seeMoreText.firstChild.data = "See Full Ingredient List";
    arrow.style.transform = "rotate(0deg)";
  } else {
    extraContent.style.display = "block";
    seeMoreText.firstChild.data = "See Less";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (window.location.hash === "#shopify-product-reviews") {
    console.log("Hash Matched");
    setTimeout(function () {
      var reviewButton = document.querySelector(
        ".spr-summary-actions-newreview button"
      );
